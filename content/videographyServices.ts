import { Language } from './about';

export const videographyServicesContent = {
  en: {
    title: "Videography Services | Regina Photo Video Studio",
    metaDescription: "Professional videography services including commercial videos, events, and creative productions.",
    heading: "Videography Packages",
    packages: [
      {
        title: "Minimum",
        description: "Perfect for short promotional videos or social media content",
        price: "€250",
        features: [
          "1 hour shooting duration",
          "up to 1 minute final video"
        ]
      },
      {
        title: "Basic",
        description: "Ideal for family stories, events, product videos, or longer promotional content",
        price: "€450",
        features: [
          "2 hours shooting duration",
          "up to 2 minutes final video"
        ]
      },
      {
        title: "Events",
        description: "Great for conferences, celebrations, or corporate events",
        price: "€600",
        features: [
          "4 hours shooting duration",
          "up to 5 minutes final video"
        ]
      }
    ],
    includedFeatures: [
      "Professional video equipment",
      "Post production",
      "Music licensing"
    ],
    additionalFees: {
      title: "Additional Fees",
      travelFee: {
        label: "Travel Fee:",
        text: "€0.40 per kilometer from Beverwijk will be added to the package price."
      },
      confidentialityFee: {
        label: "Confidentiality Fee:",
        text: "A fee of €50 applies if you wish to prevent the video from being used in my portfolio. This ensures your complete privacy while allowing me to maintain a fair pricing structure."
      }
    },
    customPackage: {
      text: "Need a custom package?",
      link: "Contact me",
      suffix: "to discuss your specific requirements."
    },
    cta: "Book Now"
  },
  nl: {
    title: "Videografie Diensten | Regina Photo Video Studio",
    metaDescription: "Professionele videografie diensten inclusief commerciële video's, evenementen en creatieve producties.",
    heading: "Videografie Pakketten",
    packages: [
      {
        title: "Minimum",
        description: "Perfect voor korte promotievideo's of social media content",
        price: "€250",
        features: [
          "1 uur opnametijd",
          "tot 1 minuut eindvideo"
        ]
      },
      {
        title: "Basis",
        description: "Ideaal voor familieverhalen, evenementen, productvideo's of langere promotiecontent",
        price: "€450",
        features: [
          "2 uur opnametijd",
          "tot 2 minuten eindvideo"
        ]
      },
      {
        title: "Evenementen",
        description: "Perfect voor conferenties, vieringen of bedrijfsevenementen",
        price: "€600",
        features: [
          "4 uur opnametijd",
          "tot 5 minuten eindvideo"
        ]
      }
    ],
    includedFeatures: [
      "Professionele videoapparatuur",
      "Post-productie",
      "Muzieklicenties"
    ],
    additionalFees: {
      title: "Extra Kosten",
      travelFee: {
        label: "Reiskosten:",
        text: "€0,40 per kilometer vanaf Beverwijk wordt aan de pakketprijs toegevoegd."
      },
      confidentialityFee: {
        label: "Vertrouwelijkheidskosten:",
        text: "Een toeslag van €50 is van toepassing als u niet wilt dat de video in mijn portfolio wordt gebruikt. Dit zorgt voor uw volledige privacy terwijl ik een eerlijke prijsstructuur kan handhaven."
      }
    },
    customPackage: {
      text: "Behoefte aan een aangepast pakket?",
      link: "Neem contact op",
      suffix: "om uw specifieke wensen te bespreken."
    },
    cta: "Nu Boeken"
  },
  ru: {
    title: "Услуги Видеосъёмки | Regina Photo Video Studio",
    metaDescription: "Профессиональные услуги видеосъёмки, включая коммерческие видео, мероприятия и креативные проекты.",
    heading: "Пакеты Видеосъёмки",
    packages: [
      {
        title: "Минимальный",
        description: "Идеально для коротких промо-видео или контента для соцсетей",
        price: "€250",
        features: [
          "1 час съёмки",
          "до 1 минуты финального видео"
        ]
      },
      {
        title: "Базовый",
        description: "Идеально для семейных историй, мероприятий, продуктовых видео или длинного промо-контента",
        price: "€450",
        features: [
          "2 часа съёмки",
          "до 2 минут финального видео"
        ]
      },
      {
        title: "Мероприятия",
        description: "Отлично подходит для конференций, праздников или корпоративных мероприятий",
        price: "€600",
        features: [
          "4 часа съёмки",
          "до 5 минут финального видео"
        ]
      }
    ],
    includedFeatures: [
      "Профессиональное видеооборудование",
      "Постпродакшн",
      "Лицензионная музыка"
    ],
    additionalFees: {
      title: "Дополнительные Расходы",
      travelFee: {
        label: "Транспортные расходы:",
        text: "€0,40 за километр от Бевервейка будет добавлено к стоимости пакета."
      },
      confidentialityFee: {
        label: "Плата за конфиденциальность:",
        text: "Доплата в размере €50 применяется, если вы не хотите, чтобы видео использовалось в моем портфолио. Это обеспечивает вашу полную конфиденциальность, позволяя мне поддерживать справедливую ценовую структуру."
      }
    },
    customPackage: {
      text: "Нужен индивидуальный пакет?",
      link: "Свяжитесь со мной",
      suffix: "чтобы обсудить ваши требования."
    },
    cta: "Забронировать"
  },
  ua: {
    title: "Послуги Відеозйомки | Regina Photo Video Studio",
    metaDescription: "Професійні послуги відеозйомки, включаючи комерційні відео, заходи та креативні проекти.",
    heading: "Пакети Відеозйомки",
    packages: [
      {
        title: "Мінімальний",
        description: "Ідеально для коротких промо-відео або контенту для соцмереж",
        price: "€250",
        features: [
          "1 година зйомки",
          "до 1 хвилини фінального відео"
        ]
      },
      {
        title: "Базовий",
        description: "Ідеально для сімейних історій, заходів, продуктових відео або довгого промо-контенту",
        price: "€450",
        features: [
          "2 години зйомки",
          "до 2 хвилин фінального відео"
        ]
      },
      {
        title: "Заходи",
        description: "Чудово підходить для конференцій, свят або корпоративних заходів",
        price: "€600",
        features: [
          "4 години зйомки",
          "до 5 хвилин фінального відео"
        ]
      }
    ],
    includedFeatures: [
      "Професійне відеообладнання",
      "Постпродакшн",
      "Ліцензійна музика"
    ],
    additionalFees: {
      title: "Додаткові Витрати",
      travelFee: {
        label: "Транспортні витрати:",
        text: "€0,40 за кілометр від Бевервейка буде додано до вартості пакету."
      },
      confidentialityFee: {
        label: "Плата за конфіденційність:",
        text: "Доплата у розмірі €50 застосовується, якщо ви не хочете, щоб відео використовувалось у моєму портфоліо. Це забезпечує вашу повну конфіденційність, дозволяючи мені підтримувати справедливу цінову структуру."
      }
    },
    customPackage: {
      text: "Потрібен індивідуальний пакет?",
      link: "Зв'яжіться зі мною",
      suffix: "щоб обговорити ваші вимоги."
    },
    cta: "Забронювати"
  }
} as const; 